...
from core.compaction import ContextProjection, ProjectedItem, ProjectionCompaction
    ...
    if nested_for:
        for stored in stored_items:
            if (
                ...
                yield _projected(stored.item, (stored.message_id,))
        return
    ...
    for stored in stored_items:
        ...
        if role == "user":
            if (m := flush()) is not None:
                yield m
            yield _projected(item, (stored.message_id,))
            continue
...
def _projected(item: TResponseInputItem, source_message_ids: tuple[int, ...]) -> ProjectedItem:
    return ProjectedItem(item=item, source_message_ids=source_message_ids)
